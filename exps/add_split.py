...
def add_split_for_person(person_id):
    ...
    for source in sources:
        ...
        logging.info(f'finish to get img {source.base_img_key}')

        crop_img = aliyun_face_detector.aliyun_human_segmentation(img)
        if crop_img:
            crop_img_key = f'{source.base_img_key[:-4]}_crop.jpg'
            ...
            db.session.commit()
            logging.info(f'add crop img {crop_img_key} for source {source.id}')
